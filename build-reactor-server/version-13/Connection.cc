...
yichen::Connection::Connection(EventLoop* loop, Socket* sock) : loop_(loop), sock_(sock) {
  if (loop_ != nullptr) {
...
void yichen::Connection::ReadNonBlocking() {
...
void 
yichen::Connection::Send(std::string msg){
    SetSendBuffer(msg.c_str());
    Write();
}

void 
yichen::Connection::Business(){
  Read();
...
void 
yichen::Connection::SetOnMessageCallback(std::function<void(Connection*)> const &callback) {
  on_message_callback_ = callback;
  std::function<void()> bus = std::bind(&yichen::Connection::Business, this);
